...
export const listEachTruthIncludesZero = (list) => {
  if (isArray(list)) {
    ...
    throw new TypeError(
      `params of listEachTruthIncludesZero is not a Array please check it,it is ${JSON.stringify(
        list
      )}`
    );
  }
...
export const paramsAllArray = (paramsList) =>
  Array.isArray(paramsList) && paramsList.every((p) => Array.isArray(p));
